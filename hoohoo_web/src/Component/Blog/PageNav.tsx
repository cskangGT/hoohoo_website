...
const Page = styled.a<PageProps>`
  ...
  display: flex;
  color: ${props =>
    props.page === props.curr ? theme.darkGray : theme.darkGray};
  text-decoration: none;
  cursor: pointer;
  border-radius: 7px;
  background-color: ${props =>
    props.page === props.curr ? theme.mainNeon : 'transparent'};
`;
    ...
    <Container>
      {props.currentPage !== 1 && (
        <Arrow onClick={handlePreviousClick}>
          <FontAwesomeIcon icon={faCaretLeft} color={theme.mainNeon} />
        </Arrow>
      )}
      <PageNumbers>
        {page.map((num, index) => (
      ...
      {props.currentPage !== page.length && (
        <Arrow onClick={handleNextClick}>
          <FontAwesomeIcon icon={faCaretRight} color={theme.mainNeon} />
        </Arrow>
      )}
    </Container>
  );

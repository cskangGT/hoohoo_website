          ...
          isOpen && <ModalBackground>
            <Wrapper>
              <ModalContent>
                ...
                {
                  deleteAccProgress === 0 ?
                  <BaseProgress setIsOpen={setIsOpen} setDeleteAccProgress={setDeleteAccProgress} />
                  : deleteAccProgress > 0 && 
                  <Container>
                      <ModalTitle>
                        <BackButton onClick={prevStep}>
                          <FontAwesomeIcon icon={faChevronLeft} fontSize={fontSize} color={theme.white} />
                        </BackButton>{data.emailProgress.title}
                      </ModalTitle>
                      {
                          deleteAccProgress === 1 ? <EmailProgress setDeleteAccProgress={setDeleteAccProgress} setEmail={setEmail} email={email} />
                          : deleteAccProgress === 2 ?
                          <ReasonProgress setDeleteAccProgress={setDeleteAccProgress} 
                              email={email}
                              setCategory={setCategory} setReason={setReason} reason={reason} category={category} />:
                              deleteAccProgress === 3 && <VerifyEmailProgress email={email} reason={reason} category={category} closeModal={closeManageAccModal} />
                      }
                  </Container>
                }
              </ModalContent>

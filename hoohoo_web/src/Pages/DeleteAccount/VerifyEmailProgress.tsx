...
import { getAccStatus, sendCode } from '../../api/deleteAcc';
...
function VerifyEmailProgress({email, reason, category, closeModal} : Props) {
    ...
    const [ownGroupModalVisible, setOwnGroupModalVisible] = useState<boolean>(false);
  ...
  return (
    <>
        ...
        <UnderlineButton onClick={resendVerificationCode}>{data.VerifyEmailProgress.resend}</UnderlineButton>
        <GreenLongNextButton unable={isLoading || code.length !== 4} onClick={verifyEmail}>{data.VerifyEmailProgress.delete}</GreenLongNextButton>
        {
            confirmModalVisible && <DeleteConfirmModal 
        ...
        {
            ownGroupModalVisible && <OwnGroupModal isOpen={ownGroupModalVisible} setIsOpen={setOwnGroupModalVisible} closeModal={closeModal} />
        }
    </>
  )
}

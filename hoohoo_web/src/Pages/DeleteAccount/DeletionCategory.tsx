...
const DeletionCategory = ({
  ...
  const DELETION_CATEGORIES: any = {
    LEAVING_TEMPORAILY: data.ReasonProgress.category['LEAVING_TEMPORAILY'],
    DISSATISFIED: data.ReasonProgress.category.DISSATISFIED,
    LACK_OF_TIME: data.ReasonProgress.category.LACK_OF_TIME,
    LACK_OF_NECESSITY: data.ReasonProgress.category.LACK_OF_NECESSITY,
  ...
  return (
    <Container onClick={handleSetCategory}>
      <HorizontalEndToEndView>
        ...
        <FontAwesomeIcon
          icon={categoryConstant === selectedCategory ? faCircleDot : faCircle}
          color={categoryConstant === selectedCategory ? '#0FB83F' : theme.gray}
          fontSize={fontSize}
        />
      </HorizontalEndToEndView>
    </Container>

...
const Outline = styled.button<OutlineProps>`
  color: ${props =>
    props.op === props.slideIndex ? '#F46F36' : theme.darkGray};
  background: none;
  border: none;
  cursor: pointer;
  margin-right: 10px;
...
const ImageButton = styled.button<BoxProps>`
  ...
  height: 100%;
  box-shadow: ${props =>
    props.isText ? '0px 0px 2px 2px #4f4f4f' : '3px 3px 5px 5px #888888'};
          ...
          <DescBox>
            <RightBox>
              <ImageButton onClick={handleButtonClick} isText={isText}>
                {!isText ? (
                  <ImageWrapper>
                    <Image src={imagePaths[slideIndex]} draggable="false" />
                  </ImageWrapper>
                ) : (

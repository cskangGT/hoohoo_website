...
function SelectPurpose() {
  ...
  const handleSendQuestionnaire = async () => {
    ...
    //   alert(localizedTexts.errorText.purposeNotSelected);
    //   return;
    // }
    // const data = [
    //   {
    ...
    //   },
    // ];
    // selectedPurpose.map(async (purpose: string) => {
    //   data.push({
  ...
  const handleSelectPurpose = (purpose: string) => {
    if (selectedPurpose.includes(purpose)) {
      setSelectedPurpose(prev => prev.filter(p => p !== purpose));
    } else {
      ...
      <Wrapper>
        <InnerWrapper>
          ...
          <SelectPurposeContainer>
            {localizedTexts.purposeList.map((goalItem: any) => (
              <SelectPurposeItem
                key={goalItem.value}
                title={goalItem.title}
                description={goalItem.description}
                image={goalItem.image}
                selected={selectedPurpose.includes(goalItem.value)}
                onClick={() => handleSelectPurpose(goalItem.value)}
              />
            ))}
          </SelectPurposeContainer>
